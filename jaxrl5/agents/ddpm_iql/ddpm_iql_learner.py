...
class DDPMIQLLearner(Agent):
    ...
    def create(
        ...
        score_model = TrainState.create(  # \mu_{\theta}(a_t | s_t, t)
        ...
        critic_cls = partial(StateActionValue, base_cls=critic_base_cls)  # Q(s, a)    critic_def = Ensemble(critic_cls, num=num_qs)
        ...
        value_def = StateValue(base_cls=critic_base_cls) # V(s)
    ...
    def update_actor(agent, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        noisy_actions = alpha_1 * batch["actions"] + alpha_2 * noise_sample # x_t
        ...
        if agent.actor_objective == "soft_adv":
            weights = jnp.where(
                adv > 0, agent.critic_hyperparam, 1 - agent.critic_hyperparam  # 0.7:0.3
            )
        elif agent.actor_objective == "hard_adv":
            weights = jnp.where(adv >= (-0.01), 1, 0)
        elif agent.actor_objective == "exp_adv":  # 
            weights = jnp.exp(adv * agent.policy_temperature)  # 3.0
        ...
        def actor_loss_fn(score_model_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            ...
            actor_loss = (
                ((eps_pred - noise_sample) ** 2).sum(axis=-1) * weights
            ).mean()
    ...
    def sample_implicit_policy(self, observations: jnp.ndarray):
        ...
        if self.critic_objective == "expectile":
            tau_weights = jnp.where(
                adv > 0, self.critic_hyperparam, 1 - self.critic_hyperparam
            )
            sample_idx = jax.random.choice(
                key, self.N, p=tau_weights / tau_weights.sum()
            )
            ...
            weights = (
                self.critic_hyperparam
                * jnp.abs(adv * self.critic_hyperparam)
                / jnp.abs(adv)
            )
